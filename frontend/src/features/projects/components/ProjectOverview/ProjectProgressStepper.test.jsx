...
describe('ProjectProgressStepper Component', () => {
    ...
    it('marque "Strategic Blueprint" comme complété si roi_analysis_summary existe', () => {
        ...
        const label = screen.getByText('Strategic Blueprint');
        expect(label).toHaveClass('text-neutral-900');
    });

    it('marque "Technical Blueprint" comme complété si des epics sont présentes', () => {
        const mockProject = { epics: [{ id: 1 }] };
        render(<ProjectProgressStepper project={mockProject} />);
        
        // On vérifie que le texte "2" a disparu au profit de l'icône check
        expect(screen.queryByText('2')).not.toBeInTheDocument();
        const label = screen.getByText('Technical Blueprint');
        expect(label).toHaveClass('text-neutral-900');
    });
