...
import ScrumMasterBlueprintWizard from '@/features/projects/components/ScrumMasterBlueprintWizard';

const ScrumMasterBlueprintPage = () => {
    const { id } = useParams();
    ...
    if (loading) {
        return (
            <div className="flex h-full w-full items-center justify-center bg-surface-background">
                <LoadingAnimation
                    className="w-64"
                    message="Retrieving scrum master specifications..."
                />
            </div>
        );
    }
        ...
        <div className="flex flex-col h-full bg-surface-background p-8 overflow-y-auto">
            {projectData?.backlog && projectData.backlog.length > 0 && (
                <div className="flex flex-col">
                    <h1 className="text-3xl font-black tracking-tight text-neutral-900">Scrum Master Blueprint</h1>
            ...
            <div className="w-full flex-1 flex flex-col space-y-8 bg-none">
                <div className="block animate-in fade-in zoom-in-95 duration-200">
                    {projectData?.backlog && projectData.backlog.length > 0 ? (
                        ...
                        <ScrumMasterBlueprintWizard initialData={projectData} projectId={id} />
                    )}
                </div>
            </div>
        </div>
...
export default ScrumMasterBlueprintPage;
