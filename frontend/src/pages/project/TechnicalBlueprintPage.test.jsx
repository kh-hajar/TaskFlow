...
vi.mock('@/features/projects/components/TechnicalBlueprintWizard', () => ({
...
import TechnicalBlueprintPage from './TechnicalBlueprintPage';

describe('Integration Test: Technical Blueprint Page', () => {
    ...
    const renderWithProviders = (id = '123') => {
        return render(
            <MemoryRouter initialEntries={[`/project/${id}/technical`]}>
                <Routes>
                    <Route path="/project/:id/technical" element={<TechnicalBlueprintPage />} />
                </Routes>
            </MemoryRouter>
        );
    };
    ...
    it('doit afficher le Dashboard si un backlog existe déjà', async () => {
        ...
        expect(screen.getByText(/Retrieving technical specifications/i)).toBeInTheDocument();
    ...
    it('doit afficher le Wizard si le backlog est vide', async () => {
        ...
        await waitFor(() => {
            ...
            // Le titre "Technical Blueprint" ne s'affiche pas si backlog vide selon ton code
            expect(screen.queryByText('Technical Blueprint')).not.toBeInTheDocument();
        });
    });
